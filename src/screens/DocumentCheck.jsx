...
export default function DocumentCheckScreen() {
  const { role } = useApp()
  const navigate = useNavigate()

  const counts = {
    ok: docs.filter(d => d.state === "ok").length,
    warn: docs.filter(d => d.state === "warn").length,
    missing: docs.filter(d => d.state === "missing").length,
  }
          ...
          <div className="workspace">
            <div className="page-head">
              ...
              <div className="actions">
                ...
                <button className="btn primary" onClick={() => navigate('/submission/local')}>
                  Lanjut: Plagiasi <Icon name="arrow" size={13} />
                </button>
              </div>
            </div>
