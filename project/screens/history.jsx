// screens/history.jsx — riwayat submission
const HistoryScreen = ({ role = "mahasiswa" }) => {
  const rows = (window.__submissions || []).concat([
  ...
  const _stageLabel = window.__stageLabel;
  const _stageBadge = window.__stageBadge;
  const _simBadge = window.__simBadge;
  ...
  return (
    <div className="app">
      ...
      <div className="shell">
        ...
        <div className="main">
          <SubBar crumbs={["Riwayat Submission"]} meta={`${rows.length} entri · sejak Sep 2025`} />
            ...
            <div className="card">
              <div className="card-head">
                <span style={{ display: "flex", gap: 6 }}>
                  {[["Semua", rows.length, true], ["Selesai", 4, false], ["Direview", 3, false], ["Ditolak", 1, false], ["Draft", 2, false]].map(([l,n,a]) => (
                    ...
                    {rows.map((s, i) => {
                      ...
                      return (
                        <tr key={s.id + i}>
                          ...
                          <td><span className={"badge " + _stageBadge[s.stage]}>{_stageLabel[s.stage]}</span></td>
                          <td>{s.similarity == null
                            ? <span className="muted">—</span>
                            : <span className={"badge " + _simBadge(s.similarity)}><span className="mono">{s.similarity}%</span></span>}</td>
                          <td>{externalSims[i] == null
                            ? <span className="muted">—</span>
                            : <span className={"badge " + _simBadge(externalSims[i])}><span className="mono">{externalSims[i]}%</span></span>}</td>
                          <td>
