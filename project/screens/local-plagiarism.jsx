// screens/local-plagiarism.jsx — hasil plagiasi database lokal
const LocalPlagiarismScreen = ({ role = "mahasiswa" }) => {
  const sources = [
          ...
          <div className="workspace">
            <div className="page-head">
              <div>
                ...
                <div className="desc">Sumber data: <span className="mono" style={{ color: "var(--text)" }}>Google Drive · prodi-abi@drive</span> · 2.408 file (TA prodi 2018–2025, jurnal internal, modul kuliah).</div>
              </div>
              <div className="actions">
                <button className="btn"><Icon name="download" size={13} /> Laporan</button>
                <button className="btn primary">Lanjut: Plagiasi Eksternal <Icon name="arrow" size={13} /></button>
              </div>
            </div>
                        ...
                        <tr key={s.id}>
                          ...
                          <td>
                            <div style={{ fontWeight: 500, lineHeight: 1.4, display: "flex", alignItems: "center", gap: 6 }}>
                              <span style={{ width: 8, height: 14, borderRadius: 2, background: `var(--${s.color === "s1" ? "red" : s.color === "s2" ? "amber" : s.color === "s3" ? "violet" : "green"})` }} />
                              {s.title}
                            </div>
...
window.LocalPlagiarismScreen = LocalPlagiarismScreen;
