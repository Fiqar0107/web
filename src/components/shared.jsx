...
export const Spark = ({ data, color = "currentColor", w = 64, h = 18 }) => {
...
export const Ring = ({ pct, size = 64, stroke = 6, color }) => {
...
export const Topbar = ({ active = "dashboard", role = "mahasiswa" }) => {
  ...
  return (
    <div className="topbar">
      ...
      <nav className="topnav">
        <a className={active === "dashboard" ? "active" : ""} onClick={() => navigate('/dashboard')}>Dashboard</a>
        <a className={active === "submission" ? "active" : ""} onClick={() => navigate('/new')}>Submission</a>
        <a className={active === "history" ? "active" : ""} onClick={() => navigate('/history')}>Riwayat</a>
        <a>Bantuan</a>
      </nav>
      ...
      <div className="user-pill" style={{ cursor: 'pointer' }} onClick={handleLogout} title="Klik untuk keluar">
...
export const Sidebar = ({ active = "dashboard", role = "mahasiswa" }) => {
  const navigate = useNavigate()
  const location = useLocation()

  const routeMap = role === "admin" ? adminRoutes : mahasiswaRoutes
    ...
    ? [
        ...
        },
        {
          group: "Repositori", entries: [
            { id: "repo", icon: "book", label: "Integrasi Drive" },
      ...
      {items.map((g) => (
        <div key={g.group}>
          ...
          {g.entries.map((e) => (
            <div
              key={e.id}
              className={"nav-item" + (isActive(e.id) || e.id === active ? " active" : "")}
              onClick={() => routeMap[e.id] && navigate(routeMap[e.id])}
            >
...
export const SubBar = ({ crumbs = [], meta }) => (
